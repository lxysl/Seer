...
def load_video(video_filename):
    ...
    left_video = decord.VideoReader(left_video_name)
    right_video = decord.VideoReader(right_video_name)
    video_timestamps = np.load(time_stamps_name)

    return left_video, right_video, video_timestamps

def select_video_frames(left_video, right_video, video_timestamps, hdf5_timestamps):
    # Find the indices of frames that are closest to the requested timestamps
    selected_indices = []

    if isinstance(hdf5_timestamps, np.int64):
        closest_idx = np.argmin(np.abs(video_timestamps - hdf5_timestamps))
        selected_indices.append(closest_idx)
    elif isinstance(hdf5_timestamps, list) or isinstance(hdf5_timestamps, np.ndarray):
        for ts in hdf5_timestamps:
            closest_idx = np.argmin(np.abs(video_timestamps - ts))
            selected_indices.append(closest_idx)

    selected_left_frames = left_video.get_batch(selected_indices).asnumpy()
    selected_right_frames = right_video.get_batch(selected_indices).asnumpy()
...
def select_video_frames_by_index(left_video, right_video, video_timestamps, index):
    # Find the indices of frames that are closest to the requested timestamps
...
if __name__ == "__main__":
    ...
    # load model weights
    checkpoint = torch.load("/home/gjt/lxy/Seer/checkpoints/pick_bottle_scratch_position_fix_wo_norm/best_model.pth", map_location="cpu")
    model.to(device)
    ...
    left_video, right_video, video_timestamps = load_video("/home/gjt/yanchi/teleoperation_dds/data/recordings/fix/training/2025-04-10_20-26-15/episode_000000000_top.svo2")

    hdf5_len = len(hdf5_timestamps)
    video_len = len(video_timestamps)
    ...
    all_text_list = []
    all_hand_state_list = []
    all_norm_hand_state_list = []
    all_norm_window_hand_state_list = []
    all_robot_state_list = []
    all_norm_robot_state_list = []
    ...
    left_image_list = []
    right_image_list = []
    for i, step in tqdm(enumerate(range(len(hdf5_timestamps)))):
        ...
        timestamp, state, gt_action = hdf5_timestamps[step], states[step], actions[step]
        video_idx = int(i / hdf5_len * video_len)
        images = select_video_frames_by_index(left_video, right_video, video_timestamps, video_idx)
        left_image, right_image = images["left"][0], images["right"][0]
        ...
        all_hand_state_list.append(state[:12].numpy())
        all_robot_state_list.append(state[-29:].numpy())
        if normalizer:
            all_norm_hand_state_list.append(norm_hand_state.numpy())
            all_norm_robot_state_list.append(norm_robot_state.numpy())
        ...
        if len(hand_state_list) >= seq_len:
            ...
            if control_type == "position":
                ...
                gt_action = np.concatenate([gt_action[-14:], gt_action[:12]], axis=0)

            all_pred_action_list.append(pred_action)
            all_denorm_pred_action_list.append(denorm_pred_action)
            all_gt_action_list.append(gt_action)
            ...
            if step < seq_len + 2:
                ...
                player.step(denorm_pred_action, mode=arm_mode, time=1.0)
            else:
    ...
    # save to hdf5
    with h5py.File("gr2_train_data.hdf5", "w") as f:
