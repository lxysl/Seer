...
class PickBottleDataset(Dataset):
    ...
    def _load_video_frames(self, video_filename, timestamps):
        """
        Load video frames from SVO2 file based on timestamps using pyzed.
        ...
        if video_filename not in self.video_cache:
            ...
            init_params.svo_real_time_mode = False  # Disable real-time mode for faster playback
            ...
            # Get SVO information
            svo_info = zed.get_camera_information().camera_configuration
            frame_count = zed.get_svo_number_of_frames()

            # Initialize frame data and timestamps
            frame_data = []
            video_timestamps = []
            ...
            # Loop through all frames in the SVO file
            for i in range(frame_count):
                if zed.grab() == sl.ERROR_CODE.SUCCESS:
                    # Retrieve the frame
                    zed.retrieve_image(image, sl.VIEW.LEFT)
                    frame_data.append(image.get_data())
